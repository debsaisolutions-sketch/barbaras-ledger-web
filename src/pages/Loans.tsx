...
export default function Loans() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        setLoading(true);
        const [ls, txns] = await Promise.all([getLoans(), getAllLoanTransactions()]);
        if (cancelled) return;
        setLoans(
          ls.map((l) => ({
            ...l,
            balance: calculateLoanBalance(txns.filter((t) => t.loanId === l.id)),
          }))
        );
      } catch (e) {
        if (!cancelled) alert((e as Error).message || "Could not load loans.");
      } finally {
  ...
  const filtered = loans
    ...
    .filter(
      (l) =>
        !search ||
        [l.borrowerName, l.borrowerPhone, l.borrowerEmail, l.relationship].some((f) =>
    ...
    <div>
      ...
      {filtered.length === 0 ? (
        <div className="empty-state">
          ...
          <p>
            {loans.length === 0
              ? "Add your first personal loan to get started."
              : "No loans match your filter."}
          </p>
          {loans.length === 0 && (
            <button className="btn btn-primary" style={{ marginTop: 16 }} onClick={() => navigate("/loans/new")}>
              + Add Loan
            </button>
          )}
        </div>
          ...
          <div key={l.id} className="list-item" onClick={() => navigate(`/loans/${l.id}`)}>
            ...
            <div className="item-content">
              ...
              <div className="item-subtitle">
                {l.relationship || "Personal"} · Original: {fmtCurrency(l.originalAmount)}
              </div>
            </div>
            <div className="item-right">
              <div
                className="item-amount"
                style={{ color: l.balance > 0 ? "var(--warning)" : "var(--success)" }}
              >
