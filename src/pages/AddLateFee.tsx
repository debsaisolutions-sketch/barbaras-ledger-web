...
export default function AddLateFee() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    addPropertyTransaction({
      propertyId: id!, date: form.date, type: "late_fee",
      description: "Late fee charge", chargeAmount: parseFloat(form.amount),
      paymentAmount: 0, paymentMethod: "", checkNumber: "", applyTo: "Late Fee", notes: form.notes.trim(),
    });
    refresh(); alert("Late fee added!"); navigate(`/properties/${id}`);
  };

  return (
    <div>
      <button className="back-link" onClick={() => navigate(`/properties/${id}`)}>← Back</button>
      <div className="page-header"><h2>⚠️ Add Late Fee</h2></div>
      <div className="card">
        <form onSubmit={handleSubmit}>
          ...
          <div style={{ display: "flex", gap: 12 }}>
            <button type="submit" className="btn btn-primary btn-lg">Add Late Fee</button>
            <button type="button" className="btn btn-outline" onClick={() => navigate(`/properties/${id}`)}>Cancel</button>
          </div>
        </form>
      </div>
    </div>
